...
const DEVICE_STATUS_SORT_ORDER: StudentDeviceStatus[] = ['online', 'disconnected', 'notEntered'];

const compareDeviceStatus = (a: StudentDeviceStatus, b: StudentDeviceStatus): number => {
  return DEVICE_STATUS_SORT_ORDER.indexOf(a) - DEVICE_STATUS_SORT_ORDER.indexOf(b);
};

const COMPARATOR_FUNCS: { [field in SortField]: ((a: ComputedStudentRow, b: ComputedStudentRow) => number) } = {
...
const TrackingTableHeaderCell: React.FC<TrackingTableHeaderCellProps> = ({ field, direction, currentSortedField, onClick, children }) => {
...
export const StudentTrackingTable: React.FC<StudentTrackingTableProps> = ({ sessionsByStudent, courseId, sessionId, students }) => {
  ...
  const [sortedDirection, setSortedDirection] = React.useState<SortDirection>('asc'); 
  ...
  const onClickField = React.useCallback((field: SortField) => {
    ...
    <Card>
      <Card.Body>
        <table className="table table-striped">
            <thead>
                <tr>
                    <TrackingTableHeaderCell field='name' direction={sortedDirection} currentSortedField={sortedField} onClick={onClickField}>
                      Student
                    </TrackingTableHeaderCell>
                    <TrackingTableHeaderCell field='computer' direction={sortedDirection} currentSortedField={sortedField} onClick={onClickField}>
                      Computer
                    </TrackingTableHeaderCell>
                    <TrackingTableHeaderCell field='mobile' direction={sortedDirection} currentSortedField={sortedField} onClick={onClickField}>
                      Mobile
                    </TrackingTableHeaderCell>
                    <TrackingTableHeaderCell field='flags' direction={sortedDirection} currentSortedField={sortedField} onClick={onClickField}>
                      Flags
                    </TrackingTableHeaderCell>
                </tr>
            </thead>
            <tbody>
              {sortedData.map((student) => 
                  <tr key={student._id}> 
                    <td>
                      <Link to={paths.professorInClassViewStudent({ courseId: courseId!, sessionId: sessionId!, studentId: student._id })}>{student.name}</Link>
                    </td>
                    <td>
