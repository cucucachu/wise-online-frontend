...
async function createSchool(
  ...
  const response = await backend.post("admin/createSchool", {
    name,
    setupKey,
    email,
    password,
...
async function claimProfessorAccount(
  setupKey: string,
  email: string,
  password: string
) {
  const response = await backend.post("professor/claimAccount", {
    setupKey,
    email,
    password,
  });
...
async function superCreateSchool({
  ...
  return backend.post("/super/school", {
    setupKey,
    adminEmail,
    billingType,
    unitPrice,
...
async function adminGetStudents({
  ...
  return backend.get(
    `/admin/students?page=${page}&pageSize=${pageSize}&firstName=${firstName}&lastName=${lastName}&email=${email}&orderBy=${orderBy}&order=${order}`
  );
}
...
async function adminGetProfessors({
  ...
  return backend.get(
    `/admin/professors?page=${page}&pageSize=${pageSize}&firstName=${firstName}&lastName=${lastName}&email=${email}&orderBy=${orderBy}&order=${order}`
  );
}
...
async function editCourse(courseId: string, data: CourseAPIData) {
  ...
  const response = await backend.post("professor/editCourse", {
    id: courseId,
    ...data,
  });
  handleResponse(response);
...
async function getAttendance(courseId: string, attendanceId: string) {
  return backend.get(
    `/professor/courses/${courseId}/attendances/${attendanceId}`
  );
}
...
async function editAttendance(
  ...
  return backend.post(
    `/professor/courses/${courseId}/attendances/${attendanceId}`,
    { students: studentsPresent, scheduledTime, startTime }
  );
}

async function setAttendanceReadyForIntegration(
  courseId: string,
  attendanceId: string
) {
  return backend.post(
    `/professor/courses/${courseId}/attendances/${attendanceId}/readyForIntegration`
  );
}
...
async function startAttendance(courseId: string) {
  const response = await backend.post("professor/startAttendance", {
    courseId,
  });
  return response;
}
...
async function getImage(testAttendanceId: string, imageNumber: string) {
  const response = await backend.get(
    `professor/testResults/${testAttendanceId}/images/${imageNumber}`
  );
...
async function getScreenshot(
  ...
  const response = await backend.get(
    `professor/testResults/${testAttendanceId}/screenshots/${screenshotNumber}`
  );
  const image = response.data;
  return image;
}
...
export async function professorGetCourseSessionDetail(
  courseId: string,
  sessionId: string
) {
  const response = await backend.get(
    `/professor/course/${courseId}/sessions/${sessionId}`
  );
  handleResponse(response);
  return response.data;
}

export async function professorGetCourseSessionDetailForStudent(
  courseId: string,
  sessionId: string,
  studentId: string
) {
  const response = await backend.get(
    `/professor/course/${courseId}/sessions/${sessionId}/students/${studentId}`
  );
  handleResponse(response);
  return response.data;
}
...
export async function professorStartCourseSession(
  courseId: string,
  params: {
  ...
  const response = await backend.post(
    `/professor/course/${courseId}/session`,
    params
  );
  handleResponse(response);
  return response.data;
...
export async function professorUpdateCourseSession(
  courseId: string,
  { allowedUrls }: { allowedUrls: string[] }
) {
...
async function markAttendance(classId: string, keyCode: string) {
  const response = await backend.post("student/markAttendance", {
    classId,
    keyCode,
  });
  return response;
...
async function submitConfidenceScore(
  ...
  const response = await backend.post("student/submitConfidenceScore", {
    testAttendanceId,
    confidenceScore,
    numberOfPeople,
    image,
...
async function submitScreenshot(testAttendanceId: string, screenshot: any) {
  const response = await backend.post("student/submitScreenshot", {
    testAttendanceId,
    screenshot,
  });
  return response;
...
async function submitAudio(audioFormData: any, proctorDetailsId: any) {
  const response = await backend.post(
    `/student/submitAudio/proctorDetails/${proctorDetailsId}`,
    audioFormData,
    {
...
async function submitTabs(testAttendanceId: string, tabs: any) {
  const response = await backend.post("student/submitTabs", {
    testAttendanceId,
    tabs,
  });
  return response;
...
async function submitProctoringError(
  testAttendanceId: string,
  errorMessage: string
) {
  const response = await backend.post("student/submitProctoringError", {
    testAttendanceId,
    errorMessage,
  });
  return response;
...
export async function studentJoinCourse({
  classId,
  keyCode,
}: {
  classId: string;
  ...
  const response = await backend.post("/student/courses/join", {
    classId,
    keyCode,
  });
  handleResponse(response);
...
export async function submitScreenshotForInClass(
  socketId: string,
  screenshot: any
) {
  const response = await backend.post("student/courses/submitScreenshot", {
    socketId,
    screenshot,
  });
  return response;
...
async function getTestImage(testId: any, imgNum: any) {
  const response = await backend.get(
    `/professor/testResults/${testId}/images/${imgNum}`
  );
  return response;
}

async function getStudents(school: any, professor: any, course: any) {
  const response = await backend.post("get/students", {
    school,
    professor,
    course,
  });
...
async function proctoringVerifyPrivileges({
  proctorSessionId,
  webcamPrivilege,
  screenshotPrivilege,
  microphonePrivilege,
}: any) {
  return backend.post("proctor/privileges", {
    proctorSessionId,
    webcamPrivilege,
    screenshotPrivilege,
    microphonePrivilege,
...
async function proctoringSetReferenceImage({
  proctorSessionId,
  webcamImage,
}: any) {
...
async function proctoringSubmitProctorData({
  proctorSessionId,
  webcamImage,
  screenshotImage,
  voiceDetected,
}: any) {
  return backend.post("proctor/submit", {
    proctorSessionId,
    webcamImage,
    screenshotImage,
    voiceDetected,
...
async function proctoringGetStudentTestDetailsAndImages({
  studentTestId,
  start,
  pageSize,
}: any) {
  return backend.get(
    `/proctor/studentTest/${studentTestId}/details?start=${start}&pageSize=${pageSize}`,
    {
      timeout: 0,
    }
  );
